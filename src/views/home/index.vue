  ...
  <div class="main-container">
    <div class="main-container-carWrapper">
      <div class="main-container-carWrapper-carBox">
        ...
        <div class="main-container-carWrapper-carBox-center" id="cartBox">
          <div class="main-container-carWrapper-carBox-center-productWrapper">
            <!-- 有库存 -->
            <div v-for="(item, i) in cart" v-if="item.isStock">
               <div class="main-container-carWrapper-carBox-center-productWrapper-productItem">
              ...
              <div class="main-container-carWrapper-carBox-center-productWrapper-productlimt"> {{item.joinFixedPrice ? '( 限购' + item.limitBuy + '件,超出全部恢复原价)' : ''}}</div> 
            </div>
    ...
    <div class="main-container-productWrapper" v-show="!isShowShopDiscounts && !showVipInfo && !showAddVip">
      ...
      <div class="main-container-productWrapper-paginationBox">
        <el-pagination background :total="total" :current-page="pageNo" layout="prev, pager, next" :page-size="pageSize" @current-change="handleCurrentChange"></el-pagination>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    GetOncePor: function() {
      ...
      if (self.productCode.length == 12) {
          likeSku = self.productCode.substr(0, 6);
          var strAmount = self.productCode.substr(6, 5);
          likeAmount = parseFloat((strAmount.substr(0, 3) + "." + strAmount.substr(3, 2)));
      } else {
        isDahua = false;
      }
      getBarcode();
      ...
      function getBarcode() {
        var proCode = x === 2 ? likeSku : self.productCode;
        getSKUByBarcode(proCode).then(function(response) {
          if ("success" in response.data && !response.data.success) {
            if (!isDahua || (isDahua && x == 2)) {
          ...
          if (dt.productSaleMethod == 1) {
            if (x == 2) {
              self.codeAddCart(dt, (likeAmount / dt.salePrice).toFixed(3));
            } else {
              self.codeProd = dt;
    ...
    submitDiscount() {
      ...
      if (re.test(this.inputVal) || (!this.inputType && this.inputVal * 1 === 10000)) {
        ...
        // 整单改价部分 保留2位小数
        if( this.inputVal>0 && this.inputVal< 1 && !this.inputType){
          let reg =  /^\+?(0\.\d{0,2})$/
    ...
    cart: {
      handler(newValue, oldValue) {
        ...
        cartUpDateTime = new Date() * 1;
				setTimeout(function(){
						var ele = document.getElementById('cartBox');
        ...
        if (this.isS2) {
          self.getWeiTime = setInterval(function() {
            try {
              var val = plus.android.getAttribute(wWeighDisPlays, "mStableWeight");
              var mStatus = plus.android.getAttribute(wWeighDisPlays, "mStatus");
              if (self.codeProd.measureUnit == "g") {
                self.weighNum = val;
              } else {
                self.weighNum = (val / 1000).toFixed(3);
              }
