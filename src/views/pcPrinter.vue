            ...
            <table v-for="item in printData.newOrderItem" width="100%" border="0">
                    <tr>
                         <td colspan="2">
                          <font  size="2">
                              <p>
																 <em v-if="item.salePrice==0" style="font-style: normal;">【赠品】</em>
															   {{item.productName}}
             ...
             <table width="100%" border="0">
                  <tr>
                      <td><font  size="2">商品总数</font></td>
                      <td align="right">
                        <font  size="2">{{printData.totalQuantity }}</font>
                     </td>
                  </tr>
                  <tr>
                  ...
                  <tr v-if="printData.cashierDiscountsAmount>0">
                     <td><font  size="2">整单优惠</font></td>
                     <td align="right"><font  size="2">{{printData.cashierDiscountsAmount | frmb}}</font></td>
                  </tr>
             ...
             <table width="100%" border="0">
                      ...
                      <tr v-if="printData.paymentTypeName!='预存款支付'">
                          <td><font  size="2">{{printData.paymentTypeName || payTypeText}}</font></td>
                          <td align="right"><font  size="2">{{(printData.totalAmount-printData.prepaymentsAmount).toFixed(2) | rmb}}</font></td>
                      </tr>
                      <tr v-if="printData.giveChange&&printData.giveChange>0">
                          <td><font  size="2">找零</font></td>
                          <td align="right"><font  size="2">{{printData.giveChange | rmb}}</font></td>
                      </tr>
             </table>
            <table width="100%" border="0">
                       <tr>
                        ...
                        <tr v-if="printData.nick&&printData.nick!='游客'">
                            <td><font  size="2">会员名</font></td>
                            <td align="right"><font  size="2">{{printData.realName ||printData.nick || printData.userName}}</font></td>
                        </tr>
                        <tr v-if="!printData.nick&&printData.userName!='tourist'">
                            <td><font size="2">会员名</font></td>
                            <td align="right"><font  size="2">{{printData.userName}}</font></td>
                        </tr>
              ...
              this.getMemberInfo(data.userId,data.orderId,function(){
                  if(self.printer=="web"){
                    setTimeout(function(){
                        ...
                        var uname = "";	
                        if(self.printData.nick!='游客'){
                                if(self.printData.nick){
                                        uname = self.printData.nick;
                                }else{
                                        uname = self.printData.userName;
                                }
                        }
                      ...
                      if(data.cashierDiscountsAmount>0)
                         PrintText+=bleObj.cTwoRow('整单优惠',self.frmb(data.cashierDiscountsAmount));
...
//                       if(self.memberInfo.uid!=0) PrintText+='\n剩余积分:'+self.memberInfo.integral;
                      if(self.printData.giveChange&&self.printData.giveChange>0) PrintText+=bleObj.cTwoRow('找零',self.rmb(self.printData.giveChange));
											PrintText+=bleObj.printLine('*');
											if(uname!="") PrintText+='\n会员名:'+uname;
                      if(self.memberInfo.uid!=0) PrintText+='\n预存款余额:'+self.rmb(self.memberInfo.balance);
                      if(self.memberInfo.orderIntegral>0) PrintText+='\n当前获得积分:'+self.memberInfo.orderIntegral;
           ...
           printerGo:function(id){
              if(this.printer=="web"){
                  ...
                  try {
                     ...
                     LODOP.SET_PRINT_PAGESIZE(3, 530, 1200, "xxxxxxxxxxxxx公司")
