        ...
        <div class="m28">
          <div class="orderDetails-top">
            ...
            <div class="ot-right">
              ...
              <span v-if="orderDetails.initiatorType==2">导购</span> :{{orderDetails.initiatorName}}
            </div>
          </div>
  ...
  methods: {
    refundLength: function(obj) {
      ...
      for (var i = 0; i < obj.length; i++) {
        if (obj[i].refundStatus && obj[i].refundStatus != "平台拒绝") {
          x++;
        }
      }
      return x == obj.length;
    ...
    refundDetails: function() {
      if (this.orderDetails.isOutOfSale) {
        this.$toast("已过售后维权期");
        return;
      }
      this.$router.push({
