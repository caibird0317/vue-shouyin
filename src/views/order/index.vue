  ...
  <div class="main-container">
    ...
    <div class="orderPay">
      <template v-if="!isShowGuide&&!isShowDetail">
        ...
        <div class="orderPay-main">
          ...
          <div class="orderPay-main-item" v-if="payType !== 'xj'">
      ...
      <div class="pro_detail" v-show="isShowDetail">
        <div class="detail_in">
          <div class="activities" v-if="activitiesNofisrt.length>0" style="background:transparent;border:none;">
            <ul>
              <li v-for="(item,i) in activitiesNofisrt" :class="{border:((i+1)/3<=1)&&activitiesNofisrt.length>3}" v-if="item.activeType == 'Rebate' || (item.activeType != 'Rebate' && item.selected)" style="background-color:#fff;">
                ...
                <div v-else>
                  ...
                  <p v-else-if="item.selected">-￥{{item.discountAmount}}</p>
                </div>
              </li>
    ...
    <div class="payScanMask" @click="qrfocus" v-show="scan">
      <div class="scan">
        ...
        <div class="dong">
          <div class="dongline" :class="{donglineAnimation: isDong}" :style="{background: 'url('+ require('@/assets/images/sm2.png') +')'}"></div>
        </div>
  ...
  methods: {
    ...
    render: function() {
      ...
      if (subdata.orderModel.supportWechatScan) {
        this.payType = 'wx';
      } else if(subdata.orderModel.supportAlipayScan) {
        this.payType = 'zfb';
      } else if(subdata.orderModel.offlinePayEnable) {
        this.payType = 'xj';
      }
      var numtotal = 0,
    ...
    subOrder: function() {
      ...
      for (var m = 0; m < this.activities.length; m++) {
        if (this.activities[m].activityId) {
          if (this.activities[m].activeType == "Rebate") {
            ...
            if (this.activities[m].activeType == "PlatformCoupon" || this.activities[m].activeType == "ShopCoupon") {
              if (this.activities[m].selected) {
      ...
      submitOrderAndPay(upData).then(function(response) {
        if (response.data.success) {
          successData = { integral: self.integral, isIntegral: self.isIntegral, startMoney: webTotalAmount,giveChange:self.giveChange, totalAll: self.totalAll, orderCreateTime: response.data.orderCreateTime };
          self.$router.push({ path: '/success', query: { discount: self.discountObj, fullOrderActivit: self.fullOrderActivit, payType: self.payTypeName } });
        } else {
          self.$toast(response.data.msg);
          setTimeout(function() {
